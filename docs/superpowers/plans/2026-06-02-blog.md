...
<article class="blog-card">
  {% if post.cover_image_url %}
    <a class="blog-card__cover" href="{{ post_url(post) }}">
...
{% block extra_head %}
{% set _base = request.url_root.rstrip('/') %}
<meta property="og:title" content="{{ post.title }}">
<meta property="og:description" content="{{ post.summary }}">
<meta property="og:type" content="article">
<meta property="og:url" content="{{ _base }}{{ post_url(post) }}">
{% if post.cover_image_url %}
<meta property="og:image" content="{{ _base }}{{ post.cover_image_url }}">
{% endif %}
...
<link rel="canonical" href="{{ _base }}{{ post_url(post) }}">
...
## Task 15.5: Cross-city `_shared` post route at `/blog/<slug>`

`_shared` posts surface on every city listing AND the hub but need a state-agnostic detail URL. They live at `/blog/<slug>`. Reserved prefixes (`assets`, `tag`, `feed.xml`) take precedence via Flask's URL matcher (more-specific routes win over `<slug>`).

**Files:**
- Modify: `src/docket/blog/loader.py` (reject `_shared` posts with reserved slugs)
- Modify: `src/docket/web/blog.py`
- Modify: `tests/web/test_blog_routes.py`
- Modify: `tests/unit/blog/test_loader.py`
...
def test_reserved_prefix_does_not_match_shared_post(app):
    ...
    client = app.test_client()
    # The hub tag route is registered; it must NOT be shadowed by the shared
    # post route.
    assert client.get("/blog/tag/budget").status_code == 200
```
...
```bash
pytest -q tests/web/test_blog_routes.py::test_shared_post_route tests/unit/blog/test_loader.py::test_shared_reserved_slug_rejected
```

Expected: route fails (no handler), loader test fails (no reserved-slug check).

- [ ] **Step 3: Add reserved-slug guard in the loader**

In `src/docket/blog/loader.py`, add at the top:

```python
RESERVED_SHARED_SLUGS = {"tag", "assets", "feed", "feed.xml", "internal"}
```

And inside `load_posts_from_disk`, after parsing each post and BEFORE the duplicate-slug check, add:
...
- [ ] **Step 4: Add the `_shared` post route**

In `src/docket/web/blog.py`, add (the order doesn't matter for Flask's matcher, but declare it last for human clarity):
...
```bash
pytest -q tests/web/test_blog_routes.py tests/unit/blog/test_loader.py
```
...
```bash
git add src/docket/blog/loader.py src/docket/web/blog.py tests/web/test_blog_routes.py tests/unit/blog/test_loader.py
git commit -m "feat(blog): /blog/<slug> route for _shared posts + reserved-slug guard"
```
...
<feed xmlns="http://www.w3.org/2005/Atom">
  ...
  <link href="{{ _base }}{{ feed_url }}" rel="self"/>
  <link href="{{ _base }}{{ feed_html_url }}"/>
  <id>{{ _base }}{{ feed_url }}</id>
  <updated>{{ updated_at }}</updated>
  {% for post in posts %}
  <entry>
    <title>{{ post.title }}</title>
    <link href="{{ _base }}{{ post_url(post) }}"/>
    <id>{{ _base }}{{ post_url(post) }}</id>
...
- [ ] **Step 0: Gitignore the substack exports directory**

Append to `.gitignore`:

```
# Substack cross-post exports (see scripts/blog_to_substack.py)
_exports/
```
