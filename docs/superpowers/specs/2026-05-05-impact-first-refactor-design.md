...
| 83 | Unified search vector for transition consistency | **Upgrade** the existing `agenda_items_search_update()` function (created in Migration 001 with `title + description` only) to coalesce `title + description + headline + why_it_matters + summary`. The `search_vector` column, the `idx_agenda_items_search` GIN index, and the `agenda_items_search_trigger` already exist from Migration 001 — Migration 013 reuses them. Search remains reliable for both pre-backfill (v2-only) and post-backfill (v3) items throughout the transition. Migration 013 also runs a one-time UPDATE to refresh existing rows so `summary` content is included. Migration 014 will `CREATE OR REPLACE` the function once more to drop the `summary` term when the column drops. |
...
| 91 | **DB-backed AI response cache** (replaces file cache) | New `ai_response_cache` table replaces the local `data/ai_cache/*.json` design from earlier drafts of decision #18. Schema: `(cache_key TEXT PK, model TEXT, prompt_version INT, response_json JSONB, cached_at, accessed_at)`. Why: shared across horizontal worker nodes during backfill; survives Railway container restarts without volume config; admin-queryable; reuses Migration 013 workflow. Cleanup: nightly task drops entries older than 90 days where current prompt version doesn't match. Cache key generation unchanged from decision #42 (sha256 of model + version + canonical input). |
...
| 95 | **Wave 0.6 — Historical sanity probe** | Between Wave 0 and Wave 0.5/Wave 1, a sync-API probe runs ~50 items each from 2017, 2019, 2021, 2023, 2025 (~250 items total, ~$2). Outputs three per-era metrics for review before committing to Waves 1-3: (a) classification distribution per era — flag if `is_substantive=false` rate diverges from the 2025 baseline by >15 percentage points (signals clerk-dialect drift across administrations, not real signal); (b) Stage 1 extraction completeness per era — flag if `counterparty` / `funding_source` / `dollars_amount` recall drops by >20% on older eras vs 2025; (c) Pydantic validator failure rate per era — flag if rejections run 4× the 2025 baseline. Any flagged metric gates Wave 1: tune item prompt v3 (or add era-conditional preamble) before spending the ~$90 backfill budget. Implementation: `scripts/wave_0_6_probe.py` reads from each era's `pending` items, runs Stage 1+2 sync, persists outputs to `out/wave_0_6_probe_<timestamp>.json` for admin review. Cost: ~$2, ~30 minutes. **Reinforces the deliberate split with decision #88**: Wave 0.5 is for fresh-content UX (current month, ~$8); Wave 0.6 is for historical-prompt validation (cross-era sample, ~$2). The two are not redundant — Wave 0.5 doesn't probe eras the prompts haven't seen. |
| 96 | **`extracted_facts` keys + weighted ranking added to search_vector** | Extends decision #83. New migration 015 does `CREATE OR REPLACE FUNCTION agenda_items_search_update()` with two changes: (a) include three `extracted_facts` keys — `extracted_facts->>'counterparty'`, `extracted_facts->'location'->>'address'`, `extracted_facts->'location'->>'neighborhood'` (the only free-text-searchable Stage 1 fields; the rest are enums); (b) assign explicit `setweight()` levels — weight `A` for `headline` + `extracted_facts` keys (most signal-dense), weight `B` for `title`, weight `C` for `description` + `why_it_matters`, weight `D` for legacy `summary`. Body uses layered composition: `setweight(to_tsvector('english', coalesce(NEW.headline, '')), 'A') \|\| setweight(to_tsvector('english', coalesce(NEW.extracted_facts->>'counterparty', '')), 'A') \|\| ...`. Migration 015 also runs a one-time UPDATE to refresh existing rows. **Land before the `IMPACT_FIRST_ENABLED=true` flag flip** so the trigger is correct from the moment v3 starts writing `extracted_facts` (avoids a second post-Phase-3 refresh pass). Migration 014 (Phase 4 cleanup) must preserve the JSONB keys and weights when removing the `summary` term — only the legacy field drops, not the layered composition. Search ranking gains: vendor-name and address queries surface the most-relevant rows first instead of treating all matches uniformly. |
| 97 | **OCR worker stage for `data_quality_skipped` queue** | New scheduled task `ocr_backfill` runs Tue 04:00 `America/Chicago` in the existing Railway `worker` service. Drains items in `processing_status='data_quality_skipped'` with `data_quality IN ('no_text_layer', 'image_only_pdf')` in `data_debt_priority DESC, meeting_date DESC` order (matches decisions #19, #79, #84). Two-tier pipeline: (1) **Tesseract first** (`pytesseract`, free, local) — if confidence > 70 and word count > 30, write OCR'd text to `agenda_items.raw_text`, set `data_quality='ok'`, `processing_status='pending'` so the item re-enters Stage 0 → Stage 1 → Stage 2 on the next `ai_items` run; (2) **Anthropic vision fallback** (Haiku 4.5 multimodal, ~$0.005/page) when Tesseract confidence is low or word count too thin, gated by `OCR_DAILY_BUDGET_USD` (default $5). Run cap: 200 items/run to bound spend. Healthchecks UUID slot reuses the per-task pattern from the cron worker. Migration 016 adds `ocr_method TEXT`, `ocr_confidence FLOAT`, `ocr_attempted_at TIMESTAMPTZ` columns to `agenda_items` for audit. **Critical for Phase 3 completeness**: without this, the 16,169 `data_quality_skipped` items (28% of the archive — concentrated in pre-2020 Woodfin years) never reach the LLM pipeline. Big Fish Override (decision #86) only catches keyword-flagged scans; routine pre-2020 image PDFs still need the bulk processor. |
...
| Wave 0.6 | **Historical sanity probe** — ~50 items each from 2017, 2019, 2021, 2023, 2025 via sync API; gates Wave 1 if per-era metrics diverge from 2025 baseline (decision #95) | ~250 | ~$2 | ~30 min |
...
adapter scrapes the URLs but isn't extracting items from the PDFs.
Pre-existing issue, not caused by Wave 0; Phase 3 should not include
Homewood until the ingest gap is closed.
...
def up(cur):
    ...
    # 4a. Upgrade existing search vector function — covers BOTH v2 (summary)
    # and v3 (headline + why_it_matters) content fields so search remains
    # reliable across the transition (decision #83).
    # Migration 001 already creates: agenda_items.search_vector column,
    # idx_agenda_items_search GIN index, agenda_items_search_update() function,
    # and agenda_items_search_trigger. We REPLACE the function body to extend
    # what gets indexed; column/index/trigger names are reused.
    cur.execute("""
...
def up(cur):
    ...
    # Update the search-vector function (agenda_items_search_update was
    # extended in Migration 013 to coalesce v2 + v3 fields; here we drop
    # the v2 `summary` term BEFORE the column drops so the trigger doesn't
    # reference a missing column when its implicit row update fires).
    cur.execute("""
