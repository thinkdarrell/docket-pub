...
- **Image:** `umamisoftware/umami:postgresql-v2.20.2` (Docker Hub; v2 last-stable pinned). The `ghcr.io/umami-software/umami` repo only has old v1 tags — Umami's current builds live on Docker Hub. v3 (Boards + Session Replay) is available but unnecessary for this v1 scope; bump deliberately later if the new features become relevant.
...
2. **Create Railway service** `analytics`: Empty Service → set image to `umamisoftware/umami:postgresql-v2.20.2`, copy env vars (`DATABASE_URL=postgres://umami:...@<host>:<port>/umami?connection_limit=5`, `APP_SECRET=<generated>`, `HASH_SALT=<generated>`).
