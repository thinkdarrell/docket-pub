...
- **#57 (BLOCKS FINAL-3)** — v3 `ai_items` hangs deterministically after 2 successful Anthropic API calls. Observed 2026-05-11 during the first FINAL-3 attempt: flag flipped at 19:11 UTC, worker restarted clean, `--run-once ai_items` made 2 successful POST calls to `api.anthropic.com/v1/messages` at 19:13:09 and 19:13:11, then blocked indefinitely. Process state at +15 min: alive, S (sleeping), RSS 218 MB (cap is 954 MB so no OOM), single thread, 0.94 CPU-sec total. Blocked in syscall=`7` (`poll()`) with timeout=`-1` (infinite) on 1 fd. 4 sockets open (2 likely DB, 2 likely Anthropic). `ai_runs.id=13` row created at 19:13:08 with `finished_at=NULL`, `rows_processed=0`. Zero `agenda_items` had v3 outputs (`extracted_facts`, `headline`, `why_it_matters` all 0) and zero `agenda_item_badges` inserted. No Anthropic SDK retry log emitted, so SDK didn't observe a timeout. SIGKILL'd by container restart on rollback — no traceback flushed. Investigation starting points: `src/docket/ai/pipeline.py` (B5 orchestrator), `extraction.py`, `rewrite.py`. Hypotheses to test: (a) Anthropic client created with `timeout=None` somewhere in the v3 path; (b) the atomic-commit DB transaction holds a lock that conflicts with the daemon's idle DB connection; (c) async/sync mix in the orchestrator deadlocks on the third operation. Repro suggestion: `PYTHONUNBUFFERED=1` + httpx DEBUG logging + run locally against prod Anthropic key on a single agenda_item, with `IMPACT_FIRST_ENABLED=true`. Rolled back to `IMPACT_FIRST_ENABLED=false` at 2026-05-11 19:35 UTC; no data corruption because v3 wrote nothing.
