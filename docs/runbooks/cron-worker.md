...
3. **Variables tab:** copy from `docket-web` via Raw Editor. `DATABASE_URL` should be `${{Postgres.DATABASE_URL}}` (reference, not literal). Add the six `HEALTHCHECK_*_UUID` vars.
