...
docket-pub/
  src/docket/              # Main package
    ...
    worker/                # Cron worker (Railway `worker` service, runs `python -m docket.worker.scheduler`)
      ...
      tasks.py             # 10 task wrappers (ingest_all, ai_items, ai_meetings, vote_matching, repair_empty_agendas, process_badges, calibration_report, process_batches, refresh_backfill_ratio_mv, prune_analytics) calling existing services; _safe_run handles Healthchecks ping + exception swallow
...
- **Cron worker:** Railway `worker` service (separate from `docket-web`, same image) runs `python -m docket.worker.scheduler` 24/7. Ten tasks staggered hourly in `America/Chicago`: `prune_analytics` day=1 04:00 (monthly), `repair_empty_agendas` Mon 05:00, `ingest_all` 06:00, `ai_items` 07:00, `ai_meetings` 08:00, `vote_matching` 09:00, `process_badges` 09:30, `calibration_report` 11:00, `process_batches` :00 and :30 (every 30 min), `refresh_backfill_ratio_mv` 04:30. Each pings Healthchecks.io start/success/fail with traceback body on exception (UUIDs in env vars; `HEALTHCHECK_PRUNE_ANALYTICS_UUID` is the newest, may be unset — task gracefully no-ops the ping when env var is missing). `BudgetExceededError` swallowed in AI tasks — expected behavior, not failure. Per-city ingest failures isolated (Birmingham failing won't block Mobile). Manual triggers via `railway ssh --service worker` then `python -m docket.worker.scheduler --run-once <task>` — NOT `railway run` (which executes locally where `postgres.railway.internal` doesn't resolve). Runbook at `docs/runbooks/cron-worker.md`.
