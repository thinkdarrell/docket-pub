...
| Impact-First Refactor — Phase 2 | ~64% — In progress | v3 pipeline (Stage 1 extraction + Stage 2 Smart Brevity rewrite + Stage 2.5 score floors + reconcile), 7 process badges + 4 BHM policy badges, Smart Brevity Card UI (6 variants), category landing pages, admin views, backfill driver. 3-track decomposition (`docs/superpowers/plans/2026-05-06-impact-first-phase-2-coordination.md`). **Track 1 done** (Section A — Stage 1 extraction worker, on `feat/impact-first-phase-2`). **Track 2 done** (Sections B1-B4 + C process badges + D policy badges, on `feat/impact-first-phase-2-track-2`). **Track 3 ~65% done** (on `feat/impact-first-phase-2-track-3`): A8 cross-task + Section E (E1-E6 Smart Brevity Card + SMART_BREVITY_UI flag) + F1 (`list_items_by_badge` service + helpers) + F2 (category landing route + template) shipped as of 2026-05-09 (HEAD `6b1378e`). Remaining in Track 3: F3 SVG volume timeline, F4 cross-filter HTMX dropdown + homepage, F5 public data-debt page + RSS, G1-G4 admin views. Then B5 atomic process_item() convergence wires Tracks 1+2+3 together; Section H backfill driver foundations; FINAL-1 through FINAL-4 deploy + flag flip. |
