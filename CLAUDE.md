...
| Impact-First Refactor — Phase 2 | **CODE-COMPLETE** (100% — pending merge to main) | v3 pipeline (Stage 1 extraction + Stage 2 Smart Brevity rewrite + Stage 2.5 score floors + reconcile + atomic-commit-with-badges), 7 process badges + 4 BHM policy badges, Smart Brevity Card UI (6 variants), category landing pages, admin views, backfill driver, B5 per-item orchestrator. 3-track decomposition merged on integration branch `release/impact-first-v3` (~/docket-pub-integration). **Track 1 done** (Section A — Stage 1 extraction worker, on `feat/impact-first-phase-2`). **Track 2 done** (Sections B1-B4 + C process badges + D policy badges, on `feat/impact-first-phase-2-track-2`). **Track 3 17/17 (100%)** + B5 wiring on `release/impact-first-v3` HEAD `386ca1e` as of 2026-05-11: A8 cross-task + Section E + Section F complete + Section G complete + Migration 016 (audit FK) + Migration 013 cycle fix (task #49 resolved) + **B5 atomic `process_item()` orchestrator** (`src/docket/ai/pipeline.py` with `process_item` + `_rerun_from_stage2`, both decision-#13 TOCTOU-guarded via `expected_status` parameter; admin paths pass `already_retried=True` to preserve one-shot semantics per decision #14b A1) + `IMPACT_FIRST_ENABLED` flag in `config.py` (defaults False) + `_process_items_v3` worker dispatch. Suite: 1305 passed + 4 xfailed (with two pre-existing environmental deselects: G1 date flake task #48 + Anthropic API key env test). Remaining: integration PR merge to main → FINAL-1 (deploy with flag off) → FINAL-2 (confidence window) → FINAL-3 (flip `IMPACT_FIRST_ENABLED=true`) → Phase 3 backfill (~$100, 7-14 days, ~37K items via Anthropic Batches API) → `SMART_BREVITY_UI=true` flips citizen rendering → Phase 4 / Migration 014 drops legacy `summary` column. **Task #52 (Grafana zero-spend alert audit) blocks FINAL-3**; task #53 (live smoke test) must run before FINAL-3. |
| Impact-First Refactor — Phase 3 | Not built | Backfill execution: ~$100 over 7-14 days for ~37K LLM-eligible items via Anthropic Batches API. Plan exists at `docs/superpowers/plans/2026-05-06-impact-first-refactor-phase-3.md`. |
