...
| Impact-First Refactor — Phase 2 | ~94% — In progress | v3 pipeline (Stage 1 extraction + Stage 2 Smart Brevity rewrite + Stage 2.5 score floors + reconcile), 7 process badges + 4 BHM policy badges, Smart Brevity Card UI (6 variants), category landing pages, admin views, backfill driver. 3-track decomposition (`docs/superpowers/plans/2026-05-06-impact-first-phase-2-coordination.md`). **Track 1 done** (Section A — Stage 1 extraction worker, on `feat/impact-first-phase-2`). **Track 2 done** (Sections B1-B4 + C process badges + D policy badges, on `feat/impact-first-phase-2-track-2`). **Track 3 16/17 done (~94%)** (on `feat/impact-first-phase-2-track-3`, HEAD `a50a464` as of 2026-05-10): A8 cross-task + Section E (E1-E6 Smart Brevity Card + SMART_BREVITY_UI flag) + Section F complete (F1 `list_items_by_badge` + F2 category landing route/template + F3 SVG volume timeline + F4 cross-filter HTMX dropdown + Browse-by-Priority + F5 public data-debt page + RSS feeds) + Section G complete (G1 calibration dashboard + G2 admin OCR queue + errors queue + G3 badge audit log viewer + manual badge HTMX endpoints + Migration 016 audit FK ON DELETE SET NULL + G4 cross-stage conflict resolution UI with 4 HTMX-driven resolution actions + `FOR UPDATE OF ai` serialization + TOCTOU guards on all persistence UPDATEs). Suite: 1166 passed + 4 xfailed (with two deselects: G1 date flake `test_query_c_returns_weeks_of_data` task #48, plus G4-surfaced `test_013_up_down_up_cycle` task #49 — migration 013 down/up cycle silently reverts post-013 FK relaxations). Remaining in Track 3: **B5 atomic process_item() convergence** wires Tracks 1+2+3 together (will subsume G4's `_rerun_stage2` minimal helper); Section H backfill driver foundations; FINAL-1 through FINAL-4 deploy + flag flip. |
